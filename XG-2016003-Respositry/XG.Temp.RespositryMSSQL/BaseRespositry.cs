...
namespace XG.Temp.RespositryMSSQL
{
    ...
    public class BaseRespositry<T> : IRespositry.IBaseRespositry<T> where T : class,new()
    {
        /// <summary>
        /// EF上下文对象
        /// </summary>
        ...
        {
            db.Set<T>().Add(model);
            return -1;//保存成功后，会将自增的id设置给 model的 主键属性，并返回受影响行数
        }
        #endregion
        ...
        public IQueryable<T> GetListBy(Expression<Func<T, bool>> whereLambda = null)
        {
            ...
            return db.Set<T>().AsNoTracking().AsExpandable().Where(whereLambda);
        }
        #endregion
        ...
        public IQueryable<T> GetListBy<TKey>(Expression<Func<T, bool>> whereLambda, Expression<Func<T, TKey>> orderLambda)
        {
            ...
            return db.Set<T>().AsNoTracking().AsExpandable().Where(whereLambda).OrderBy(orderLambda);
        }
        #endregion
        ...
        public IQueryable<T> GetPagedList<TKey>(int pageIndex, int pageSize, Expression<Func<T, bool>> whereLambda, Expression<Func<T, TKey>> orderBy)
        {
            ...
            return db.Set<T>().AsExpandable().AsNoTracking().Where(whereLambda).OrderBy(orderBy).Skip(pageIndex).Take(pageSize);
        }
        #endregion
        ...
        {
            rowCount = db.Set<T>().AsNoTracking().AsExpandable().Where(whereLambda).Count();
            //1.查询分页数据
            if (isAsc)
            {
                return db.Set<T>().AsNoTracking().AsExpandable().Where(whereLambda).OrderBy(orderBy).Skip(pageIndex).Take(pageSize);
            }
            //2.查询总行数
            else
            {
                return db.Set<T>().AsNoTracking().AsExpandable().Where(whereLambda).OrderByDescending(orderBy).Skip(pageIndex).Take(pageSize);
            }
        }
        #endregion
        public int GetProcedureByOne(string commandText, params Object[] parameters)
        {
            int results = db.Database.SqlQuery<int>(commandText, parameters).FirstOrDefault();
            return results;
        }

        public IList<T> GetProcedureList<T>(string commandText, params Object[] parameters)
        {
            IList<T> results = db.Database.SqlQuery<T>(commandText, parameters).ToList<T>();
            return results;
        }

        public int ExecteNonQuery(string commandText, params Object[] parameters)
        {
            return db.Database.ExecuteSqlCommand(commandText, parameters);
        }
    }
}
